...
export const orderType = defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      type: "string",
      validation: (Rule) => Rule.required(),
      description: "Firebase UID of the user who placed this order",
    }),
    defineField({
      name: "clerkPaymentId",
    ...
    defineField({
      ...
      type: "string",
      hidden: ({ document }) => document?.paymentMethod !== "midtrans",
    }),
    defineField({
      name: "midtransSessionKey",
      title: "Midtrans Session Key",
      type: "string",
      hidden: ({ document }) => document?.paymentMethod !== "midtrans",
    }),
    defineField({
      name: "midtransValidationId",
      title: "Midtrans Validation ID",
      type: "string",
      hidden: ({ document }) => document?.paymentMethod !== "midtrans",
    }),
    defineField({
      name: "midtransBankTransactionId",
      title: "Midtrans Bank Transaction ID",
      type: "string",
      hidden: ({ document }) => document?.paymentMethod !== "midtrans",
    }),
    defineField({
      name: "midtransCardType",
      title: "Midtrans Card Type",
      type: "string",
      hidden: ({ document }) => document?.paymentMethod !== "midtrans",
    }),
    defineField({
      name: "midtransCardIssuer",
      title: "Midtrans Card Issuer",
      type: "string",
      hidden: ({ document }) => document?.paymentMethod !== "midtrans",
    }),
    defineField({
      name: "midtransCardBrand",
      title: "Midtrans Card Brand",
      type: "string",
      hidden: ({ document }) => document?.paymentMethod !== "midtrans",
    }),
    defineField({
      name: "midtransCardNo",
      title: "Midtrans Card Number (Masked)",
      type: "string",
      hidden: ({ document }) => document?.paymentMethod !== "midtrans",
    }),
    defineField({
      name: "midtransTransactionDate",
      title: "Midtrans Transaction Date",
      type: "string",
      hidden: ({ document }) => document?.paymentMethod !== "midtrans",
    }),
    defineField({
    ...
    defineField({
      ...
      type: "number",
      description: "Total discount from product-level discounts",
      initialValue: 0,
    }),
    defineField({
      ...
      type: "number",
      description: "2% discount for business accounts",
      initialValue: 0,
    }),
    defineField({
      name: "coupon",
      ...
      fields: [
        ...
        defineField({
          ...
          type: "number",
          description:
            "Percentage (e.g., 20 for 20%) or fixed amount (e.g., 10 for $10)",
        }),
        defineField({
          name: "discountAmount",
          title: "Discount Amount Applied",
          type: "number",
          description: "Actual discount amount applied to this order",
        }),
        defineField({
          name: "couponRef",
      ...
      description: "Coupon applied to this order with discount details",
      preview: {
        select: {
          code: "code",
        ...
        prepare(select) {
          const typeDisplay =
            select.discountType === "percentage"
              ? `${select.discountValue}% off`
              : `$${select.discountValue} off`;

          return {
            title: `🎟️ ${select.code}`,
            subtitle: `${typeDisplay} | Saved: $${
              select.discountAmount?.toFixed(2) || 0
            }`,
          };
        },
      },
