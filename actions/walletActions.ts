...
export async function getUserWalletBalance(): Promise<{
  ...
  try {
    ...
    const user = await backendClient.fetch(
      `*[_type == "user" && firebaseUid == $firebaseUid][0]{ walletBalance }`,
      { firebaseUid: userId },
    );
...
export async function getWalletTransactions(): Promise<{
  ...
  try {
    ...
    const user = await backendClient.fetch(
      `*[_type == "user" && firebaseUid == $firebaseUid][0]{ walletTransactions }`,
      { firebaseUid: userId },
    );
 ...
 *
 * Uses Sanity's atomic `.inc()` operator wrapped in a transaction with the
 * audit-trail array insert so two concurrent credits cannot lose each
 * other (the previous read-modify-write pattern was vulnerable to lost
 * updates). `balanceBefore` / `balanceAfter` in the audit record are
 * best-effort snapshots — `delta` is the source of truth.
 */
export async function addWalletCredit(
  ...
  try {
    ...
    const user = await backendClient.fetch(
      `*[_type == "user" && firebaseUid == $userId][0]{ _id, walletBalance }`,
      { userId },
    );
    ...
    await backendClient
      .patch(user._id)
      .setIfMissing({ walletBalance: 0, walletTransactions: [] })
      .inc({ walletBalance: amount })
      .insert("before", "walletTransactions[0]", [transaction])
 ...
 *
 * Uses optimistic locking via `ifRevisionId` so two concurrent debits
 * cannot both succeed and overdraw the wallet. On revision conflict we
 * retry a small number of times; if the balance has truly dropped below
 * the requested amount, we surface "Insufficient balance" instead.
 */
export async function deductWalletBalance(
  ...
  try {
    ...
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
      ...
      try {
        await backendClient
          .patch(user._id, { ifRevisionID: user._rev })
          .setIfMissing({ walletBalance: 0, walletTransactions: [] })
          .dec({ walletBalance: amount })
          .insert("before", "walletTransactions[0]", [transaction])
          .commit({ autoGenerateArrayKeys: true });
        ...
        lastError = err;
        // Sanity raises a 409 on ifRevisionID mismatch — retry the read.
        const message = (err as { message?: string })?.message ?? "";
        if (!/revision|conflict|409/i.test(message)) throw err;
      }
    }

    console.error("deductWalletBalance: exhausted retries", lastError);
    return { success: false, message: "Wallet busy, please retry" };
  } catch (error) {
...
export async function getWithdrawalRequests(): Promise<{
  ...
  try {
    ...
    const user = await backendClient.fetch(
      `*[_type == "user" && firebaseUid == $userId][0]{ withdrawalRequests }`,
      { userId },
    );
